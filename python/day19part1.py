#!/usr/bin/python3
# Python solution for AOC 2021 Day 19 Part 1
#
...
if __name__ == "__main__" :
    ...
    for fromScanner in processScannerList :
        toScanner = zeroChains[fromScanner][0]
        transform = scannerOffsets[toScanner][fromScanner]
        print(f"Transforming scanner {fromScanner}({len(dataList[fromScanner])}) to {toScanner}({len(dataList[toScanner])}) and merging:",end=" ")
        dataList[toScanner] = transformAndMerge(transform, dataList[fromScanner], dataList[toScanner])
        print(f"{len(dataList[toScanner])} merged probes")


    print(f"If you're still awake, the Part One Answer you seek is now: {len(dataList[0])}")
